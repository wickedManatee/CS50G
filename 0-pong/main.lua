...
function love.load()
	...
	-- number of ai players in the game (0, 1, 2)
	aiPlayers = -1
...
function love.update(dt)
    ...
    --
    -- player 1
	if not player1.isComputer then
		...
		if (player1.y+(player1.height/2)) > ball.y then
			player1.dy = -PADDLE_SPEED
	...
	-- player 2
	if not player2.isComputer then
...
function love.keypressed(key)
	...
	elseif key == 'enter' or key == 'return' then
        if gameState == 'start' and aiPlayers > -1 then
            gameState = 'serve'
...
function love.draw()
    ...
    if gameState == 'start' then
        ...
        love.graphics.printf('Press the number key (0,1,2) for the number of computer players. Then, press Enter to begin!', 0, 20, VIRTUAL_WIDTH, 'center')
    elseif gameState == 'serve' then
        -- UI messages
        love.graphics.setFont(smallFont)
