...
function Level:init()
    ...
    function beginContact(a, b, coll)
        ...
        if types['Obstacle'] and types['Player'] then
            ...
            if a:getUserData() == 'Obstacle' then
				...
				table.insert(self.playersCollided, b:getBody())
            else
				...
				table.insert(self.playersCollided, a:getBody())
            end
        end
	...
	table.insert(self.obstacles, Obstacle(self.world, 'vertical',
        VIRTUAL_WIDTH - 110, VIRTUAL_HEIGHT - 35 - 110 - 110))
    table.insert(self.obstacles, Obstacle(self.world, 'vertical',
        VIRTUAL_WIDTH - 45, VIRTUAL_HEIGHT - 35 - 110 - 110))
    table.insert(self.obstacles, Obstacle(self.world, 'horizontal',
        VIRTUAL_WIDTH - 80, VIRTUAL_HEIGHT - 35 - 110 - 35 - 110 - 35/2))
    ...
    table.insert(self.aliens, Alien(self.world, 'square', VIRTUAL_WIDTH - 80, VIRTUAL_HEIGHT - TILE_SIZE - ALIEN_SIZE / 2 - 300, 'Alien'))
...
function Level:update(dt)
	...
	for k, body in pairs(self.playersCollided) do
		for j, alien in pairs(self.launchMarker.alienz) do
			if body == alien.body then
				alien.collided = true
			end
		end
    end
    ...
    if self.launchMarker.launched then
		for k, player in pairs(self.launchMarker.alienz) do
			...
			if xPos < 0 or (math.abs(xVel) + math.abs(yVel) < 3) then
				table.remove(self.launchMarker.alienz, k)
				player.body:destroy()				
			end
		end
		if #self.launchMarker.alienz == 0 then
			self.launchMarker = AlienLaunchMarker(self.world)
		end
